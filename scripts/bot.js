...
function product(iterable, repeat) {
    return Array.from({ length: repeat }, () => Array.from(iterable)).reduce(
        (a, b) =>
            a.flatMap((x) =>
            b.map((y) => (Array.isArray(x) ? [...x, y] : [x, y]))
        )
    );
}

const possibleWordsFromGuesses = (guesses, wordList) => {
    let possibleWords = [];
    for (let word of wordList) {
        let valid = true;
        for (let guess of guesses) {
            let wordToChange = word;
            let greens = guess.result.green
            let yellows = guess.result.yellow
            let greys = Array.from(
                { length: word.length },
                (value, index) => index
            );

            for (let green of greens) {
                if (guess.word[green] != wordToChange[green]) {
                    valid = false;
                    break;
                }
                else {
                    if (green === wordToChange.length - 1) {
                        wordToChange = wordToChange.slice(0, green) + ' '
                    }
                    else {
                        wordToChange = wordToChange.slice(0, green) + ' ' + wordToChange.slice(green + 1, wordToChange.length)
                    }
                greys.splice(greys.indexOf(green), 1)
                }
            }
            if (!valid) break;
            else {
                for (let yellow of yellows) {
                    if (!wordToChange.includes(guess.word[yellow]) || guess.word[yellow] === wordToChange[yellow]) {
                        valid = false;
                        break;
                    }
                    else {
                        wordToChange = wordToChange.replace(guess.word[yellow], ' ')
                    }
                    greys.splice(greys.indexOf(yellow), 1)
                }
                if (!valid) break;
                else {
                    for (let grey of greys) {
                        if (wordToChange.includes(guess.word[grey])) {
                            valid = false;
                            break;
                        }
                    }
